...
def data_shape(df):
    ...
    for i in range(0, len - 80, 40):
        if df.loc[i, 'class_num'] == label_index and df.loc[i + 80, 'class_num'] == label_index:  # 50개의 6새센서 데이터가 한 세트
            print("save")
        ...
        if df.loc[i, 'class_num'] == label_index and \
                ...
                df.loc[i + 40, 'class_num'] == df.loc[i + 79, 'class_num']:  # 50개의 6새센서 데이터가 한 세트
            print("change"+str(a))
            a+=1

            label_index = df.loc[i + 79, 'class_num']
            # print(label_index)
