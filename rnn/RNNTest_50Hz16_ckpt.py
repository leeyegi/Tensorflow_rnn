...
cost_pre=tf.nn.softmax_cross_entropy_with_logits_v2(logits=model, labels=Y, name='Y')
...
def run_train(session, train_x, train_y):
    ...
    for epoch in range(total_epoch):
        ...
        for i in range(total_batch):
            ...
            _, acc_train_result, cost_train_result = session.run([optimizer, cost, accuracy], feed_dict={X: batch_x, Y: batch_y})
