...
result = txt.getmany([["dataset/1g2018041952.txt", "dataset/1a2018041952.txt", "dataset/1t2018041952.txt"],
                      ["dataset/2g201804091947.txt","dataset/2a201804091947.txt","dataset/2t201804091947.txt"],
                      ["dataset/3g20180409131241.txt","dataset/3a20180409131241.txt","dataset/3t20180409131241.txt"],
                      ["dataset/4g20180409455.txt", "dataset/4a20180409455.txt", "dataset/4t20180409455.txt"],
                      ["dataset/5g201804092011.txt", "dataset/5a201804092011.txt", "dataset/5t201804092011.txt"]
                      ])
...
dataset_y_onehot[np.arange(1226), dataset_y] = 1
'''
...
# 옵션 설정
#########
learning_rate = 0.0005              #현재 신경망이 단순하기 떄문에 학습이 금방끝나므로 학습률을 낮게 조정
total_epoch = 25                    #에폭 -> 훈련 데이터를 모두 소진했을때 에폭수가 1 올라감
batch_size = 1000                    #한번에 몇개의 데이터를 학습할지 배치사이즈 설정
...
for epoch in range(total_epoch):
    ...
    # 한 에폭당 total_batch만큼 학습
    for i in range(total_batch):
        #학습을 위한 훈련데이터를 배치 사이즈 만큼 뽑아서 저장
        #
        batch_xs, batch_ys=train_data_x, train_data_y
        '''
        print(batch_xs.shape)
        print(batch_ys.shape)
        batch_xt, batch_yt=test_data_y, test_data_y
        '''
    ...
    print('Epoch:', '%04d' % (epoch + 1), '정확도:', train_acc, 'Avg. cost =', '{:.3f}'.format(total_cost / total_batch))
